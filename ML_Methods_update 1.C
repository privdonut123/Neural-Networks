...
int ML_Methods(TString myMethodList = "")
{
   ...
   Use["MLP_Default"]     = 0; // Recommended ANN
   Use["MLP_Tanh"]        = 0;
   Use["MLP_Sigm"]        = 0;
   Use["DNN_Tanh"]        = 0;
   Use["DNN_Sigm"]        = 0;
   ...
   Use["BDT_10_K"]             = 1; // uses Adaptive Boost
   ...
   std::cout << "--- TMVAClassification       : Using signal input file: " 
             << input_s->GetName() << std::endl 
             << "--- TMVAClassification       : Using background input file: "
             << input_b->GetName() << std::endl;
   ...
   if (Use["MLP_Tanh"])
                      factory->BookMethod( dataloader, TMVA::Types::kMLP, "MLP_Tanh",
                                           "NeuronType=tanh:HiddenLayers=12,11");
         
   if (Use["MLP_Sigm"])      
                      factory->BookMethod( dataloader, TMVA::Types::kMLP, "MLP_Sigm",
                                           "NeuronType=sigmoid:HiddenLayers=12,11");
                   
            

   // Multi-architecture DNN implementation.
   if (Use["DNN_Tanh"] or Use["DNN_Sigm"] or Use["DNN_Sigm_Fast"])
      {
      // General layout.
   ...
   if (Use["BDT_10_K"])
      ...
      factory->BookMethod(dataloader, TMVA::Types::kBDT, "BDT_10_K",
                           "!H:!V:NTrees=10:MinNodeSize=2.5%:MaxDepth=3:"
    ...
    */
    ...
    TCanvas *roc_graph = factory->GetROCCurve(dataloader);
    roc_graph->Draw();
                    ...
                    for(int i = 0; i != arr_size; i++)
                        {
                            ...
                            roc_data << left << setw(dent*dent_mult) 
                            << xp << left << setw(dent*dent_mult)
                            << yp << endl;
                        }
                    dent_mult++;
    ...
    if (Method_T_or_F == false)
        cout << "None of the methods have been selected! ML_Methods_ROCdata_C++.txt is empty."
             << endl;
    
   //roc_data.open("ML_Methods_ROCdata_C++.txt", ios::app);
   ...
   gROOT->ProcessLine("cout << \"Dio says WRYY!\" << endl;");
   return 0;
